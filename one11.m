%% Part 2: Feature Extraction using EWT (Empirical Wavelet Transform) and HT
clc; clear; close all;
...
for c = 1:length(unique_classes)
    ...
    val_count = round(length(indices) * 0.2);
    
    train_indices = [train_indices; indices(1:train_count)];
    val_indices = [val_indices; indices(train_count+1:train_count+val_count)];
    test_indices = [test_indices; indices(train_count+val_count+1:end)];
end

% Create datasets
train_data = z(train_indices, :);
train_labels = cl(train_indices);
val_data = z(val_indices, :);
val_labels = cl(val_indices);
test_data = z(test_indices, :);
test_labels = cl(test_indices);

disp(['Training set: ', num2str(size(train_data, 1)), ' signals']);
disp(['Validation set: ', num2str(size(val_data, 1)), ' signals']);
...
fprintf('Extracting EWT-based features from validation signals...\n');
for i = 1:size(val_data, 1)
    val_features(i,:) = extract_EWT_features(val_data(i,:), fs, num_IMFs);
    if mod(i, 50) == 0 || i == size(val_data, 1)
        fprintf('Processed %d/%d validation signals with EWT\n', i, size(val_data, 1));
    end
end
...
val_features_norm = (val_features - min_vals) ./ range_vals;
...
val_features_norm(isnan(val_features_norm) | isinf(val_features_norm)) = 0;
test_features_norm(isnan(test_features_norm) | isinf(test_features_norm)) = 0;

%% Save EWT-based Features
save('PQD_features_EWT.mat', ...
     'train_features_norm', 'val_features_norm', 'test_features_norm', ...
     'train_labels', 'val_labels', 'test_labels', 'unique_classes');

fprintf('\nEWT-based feature extraction completed. Saved to PQD_features_EWT.mat\n');
